...
function headerSql(id) {
  ...
  FROM dbo.Proposal p
  LEFT JOIN dbo.AspNetUsers u ON u.Id = p.SaleId
  LEFT JOIN dbo.AspNetUsers la ON la.Id = p.LastApprovalId
  LEFT JOIN dbo.SaleOffice so ON so.SAL_OFF_CODE = p.SaleOfficeCode
  WHERE p.Id = '${id}'`);
}
...
function timelineSql(id) {
  ...
  LEFT JOIN dbo.AspNetUsers u ON u.Id = h.ApproverId
  LEFT JOIN dbo.AspNetRoles r ON r.Id = h.ApproverRoleId
...
const USERS_SQL = jsonArr(`SELECT CONVERT(NVARCHAR(36), Id) AS id, Name AS name, ISNULL(Email,'') AS email,
  ...
  FROM dbo.AspNetUsers WHERE IsDelete = 0 AND RoleCode IN ('sam','sdm','pte','cdr')`);
